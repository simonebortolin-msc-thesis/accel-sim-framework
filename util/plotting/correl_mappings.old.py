...
[
    ...
    # 1350 MHz
    CorrelStat(chart_name="Cycles",
        plotfile="titan-rtx-cycles",
        hw_eval="np.average(hw[\"Duration\"])*1350",
        hw_error="np.max(hw[\"Duration\"])*1350 - np.average(hw[\"Duration\"])*1350,"+\
                 "np.average(hw[\"Duration\"])*1350 - np.min(hw[\"Duration\"])*1350",
        sim_eval="float(sim[\"gpu_tot_sim_cycle\s*=\s*(.*)\"])",
        hw_name="NVIDIA RTX TITAN",
        drophwnumbelow=0,
        plottype="log",
        stattype="counter"
    ),
    ...
    CorrelStat(chart_name="GPC Cycles",
        plotfile="gpc_cycles",
        hw_eval="np.average(hw[\"gpc__cycles_elapsed.avg\"])*1350",
        hw_error="np.max(hw[\"gpc__cycles_elapsed.avg\"])*1350 - np.average(hw[\"gpc__cycles_elapsed.avg\"])*1350,"+\
                 "np.average(hw[\"gpc__cycles_elapsed.avg\"])*1350 - np.min(hw[\"gpc__cycles_elapsed.avg\"])*1350",
        sim_eval="float(sim[\"gpu_tot_sim_cycle\s*=\s*(.*)\"])",
        hw_name="all",
        drophwnumbelow=0,
        plottype="log",
