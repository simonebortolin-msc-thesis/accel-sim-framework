...
for bench in benchmarks:
	...
	# then, we delete the intermediate files ((.trace and kernelslist files files)
        sh_contents += "\nexport CUDA_VERSION=\"" + cuda_version + "\";\nexport CUDA_VISIBLE_DEVICES=\"" + options.device_num + "\" ;\n" +\
            "export TRACES_FOLDER="+ this_trace_folder + ";\nCUDA_INJECTION64_PATH=" + os.path.join(nvbit_tracer_path, "tracer_tool.so") + " " +\
            exec_path + " " + str(args) + " ;\n" + os.path.join(nvbit_tracer_path,"traces-processing", "post-traces-processing") + " " +\
            os.path.join(this_trace_folder, "kernelslist") #+ " ;\n rm -f " + this_trace_folder + "/*.trace ;\n rm -f " + this_trace_folder + "/kernelslist "
